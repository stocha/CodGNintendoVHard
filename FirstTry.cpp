...
const int stackSize=812000*256;
//const int stackSize=812000*64;

/* Multiples hypotheses */
struct hypStack{
    
    
    void solve(hyp *in,hyp *last,int bit,Formule *f){
        hyp curr;
        curr.copy(last);
        if(bit>=in->nbBits){
            
            //cout << "checking "; curr.out();
            for(int i=0;i<in->nbBits;i++){
                unsigned int v=in->bitAt(i);
                unsigned int vf=curr.test(&f[i]);
                if(v!=vf){ 
                    //cout<< "form "<< i << "| " << v << "!=" << vf << "for ";f[i].out();
                    return;
...
void tst004(){
    ...
    hyp *in=fromString(is);
      ...
      cout << "++++++++++ by memory next ++++++++++++"<<endl;      
      //int nni=0;int nny=0; nni=nni/nny;
}
...
void tst003(){
    ...
    hyp *in=fromString(is);
      ...
      hypStack *hh = new hypStack;
      for(int numForm=0;numForm<in->nbBits;numForm++){
